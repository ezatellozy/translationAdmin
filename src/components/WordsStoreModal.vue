    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <template v-slot:activator="{ on, attrs }">
        <v-btn color="blue" dark elevation="0" v-bind="attrs" v-on="on">
          ...
          <span class="ml-1">Word dictionary create</span>
        </v-btn>
      </template>
  ...
  methods: {
    submit() {
      ...
      data.append(
        ...
        this.invalidArabicChars.replace(/\s/g, "").split("").join(" ")
      );
      data.append(
        "invalid_english_chars",
        this.invalidEnglishChars.replace(/\s/g, "").split("").join(" ")
      );
      this.axios
        .post("admin/word-dictionaries", data)
        .then(() => {
